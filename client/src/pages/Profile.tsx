            ...
            <div className="flex flex-col items-center gap-3">
              {profile?.resumePath ? (
                <>
                  ...
                  <p className="text-xs text-slate-400">{profile.resumeText ? 'Text extracted ✓' : 'No text extracted'}</p>
                </>
              ) : (
                <>
                  <Upload className="w-8 h-8 text-slate-400" />
                  <p className="text-sm text-slate-600">Drop your resume PDF here, or</p>
                </>
              )}
              <label className="cursor-pointer text-sm text-indigo-600 hover:underline font-medium">
                {profile?.resumePath ? 'Replace resume' : 'Browse file'}
